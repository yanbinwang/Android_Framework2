...
class LoadingDialog(context: Context) : Dialog(context, R.style.Album_Dialog) {
     ...
     */
    fun setupViews(widget: Widget, @StringRes message: Int) {
        val color = if (widget.uiStyle == Widget.STYLE_LIGHT) {
            // 浅色模式 → 深色加载条
            ContextCompat.getColor(context, R.color.albumLoading)
        } else {
            // 深色模式 → 用主题色
            ContextCompat.getColor(context, widget.statusBarColor)
        }
