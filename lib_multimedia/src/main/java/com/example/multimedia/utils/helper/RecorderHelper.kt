...
class RecorderHelper(private val context: Context) {
     ...
     */
    fun startRecord() {
        val recordFile = MultimediaUtil.getOutputFile(MediaStore.Files.FileColumns.MEDIA_TYPE_AUDIO)
        ...
        try {
            ...
            recorder?.apply {
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) setOutputFile(sourcePath) else setOutputFile(
                    recordFile
                )
                prepare()
                start()
