...
abstract class BaseKLineChartView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ScrollAndScaleView(context, attrs, defStyleAttr) {
    ...
    private val mChildDraws by lazy { ArrayList<IChartDraw<Any>>() }
    private val mGridPaint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    private val mTextPaint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    private val mMaxMinPaint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    private val mBackgroundPaint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    private val mSelectedXLinePaint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    private val mSelectedYLinePaint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    private val mSelectPointPaint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    private val mSelectorFramePaint by lazy { Paint(Paint.ANTI_ALIAS_FLAG) }
    private val mDataSetObserver by lazy { object : DataSetObserver() {
    ...
    init {
        ...
        mSelectorFramePaint.strokeWidth = Dp2Px(getContext(), 0.6f).toSafeFloat()
    ...
    private fun drawText(canvas: Canvas) {
        ...
        for (i in 1 until mGridColumns) {
            ...
            if (translateX in startX..stopX) {
                ...
                val text = mAdapter?.getDate(index).orEmpty()
        ...
        if (isLongPress) {
            ...
            var x: Float
            val text: String = formatValue(point.closePrice)
    ...
    fun notifyChanged() {
        if (isShowChild && mChildDrawPosition == -1) {
            mChildDraw = mChildDraws.safeGet(0)
    ...
    private fun calculateValue() {
        ...
        for (i in mStartIndex..mStopIndex) {
            ...
            if (mMainDraw != null) {
                ...
                if (mMainHighMaxValue != mMainHighMaxValue.coerceAtLeast(point.highPrice)) {
                    mMainHighMaxValue = point.highPrice
                    mMainMaxIndex = i
                }
                if (mMainLowMinValue != mMainLowMinValue.coerceAtMost(point.lowPrice)) {
