package com.example.gallery.album.app.camera
...
import com.example.gallery.album.utils.AlbumUtil.randomJPGPath
import com.example.gallery.album.utils.AlbumUtil.randomMP4Path
import com.example.gallery.album.utils.AlbumUtil.takeImage
import com.example.gallery.album.utils.AlbumUtil.takeVideo
import java.io.File
        ...
        when (mFunction) {
            Album.FUNCTION_CAMERA_IMAGE -> {
                if (mCameraFilePath.isEmpty()) {
                    // 没有指定路径，自动生成一个
                    mCameraFilePath = randomJPGPath(this)
                }
                // 调用系统拍照
                takeImage(this, CODE_ACTIVITY_TAKE_IMAGE, File(mCameraFilePath))
            }
            Album.FUNCTION_CAMERA_VIDEO -> {
                if (mCameraFilePath.isEmpty()) {
                    // 自动生成视频路径
                    mCameraFilePath = randomMP4Path(this)
                }
                // 调用系统录像
                takeVideo(this, CODE_ACTIVITY_TAKE_VIDEO, File(mCameraFilePath), mQuality, mLimitDuration, mLimitBytes)
            }
