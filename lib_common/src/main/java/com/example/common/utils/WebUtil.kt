...
class WebUtil(host: Any, private val container: ViewGroup?) : DefaultLifecycleObserver {
    private var mActivity = when (host) {
    ...
    init {
        ...
        try {
            mWebView = WebView(BaseApplication.instance.applicationContext)
        } catch (_: RuntimeException) {
            // 这里捕捉一个webview不存在的bug
            mActivity.finish()
        }
        if (null != mWebView && !mActivity.isFinishing && !mActivity.isDestroyed) {
