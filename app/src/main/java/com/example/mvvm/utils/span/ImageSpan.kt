...
//}
/**
 * size = 18.pt->UI要求的图片大小
 */
class RankSpan(private val mImageSize:Int,private val mDifference: Int = 0) : ReplacementSpan() {
    private var mSize = 0
...
interface RankSpanInterface {
    ...
    fun Spannable.setRankSpan(size:Int): Spannable {
        if (!contains("★")) return this
        var index = -1
        for (i in 1..10) {
            index = indexOf("★", index + 1)
            if (index in indices) {
                setSpan(RankSpan(size), index, index + 1, Spanned.SPAN_INCLUSIVE_EXCLUSIVE)
            } else {
                break
            }
        }
