 ...
 *
 *   Android 14（API 级别 34）新增
 *   1. microphone
 *     说明：表示该前台服务需要持续访问麦克风进行录音。适用于语音备忘录、实时通话录音、屏幕录制（带声音采集）等场景。
 *   2. camera
 *     说明：表示该前台服务需要持续访问摄像头。适用于后台视频录制、安防监控、AR 扫描等场景。
 *   3. remoteMessaging
 *     说明：用于处理远程消息传递的前台服务，例如即时通讯应用的后台消息接收、推送保活等。
 *   4. systemExempted
 *     说明：仅限系统应用或获得特殊豁免的应用使用，用于执行不受常规 FGS 限制的系统级任务。
 *   5. shortService
 *     说明：用于执行短暂、可预期的后台任务（通常 < 3 分钟），如快速上传小文件、保存草稿等。
 */
