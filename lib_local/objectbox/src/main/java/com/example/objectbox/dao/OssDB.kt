...
data class OssDB(
    ...
    @Index(type = IndexType.VALUE) var userId: String? = null, // 当前用户的id (区别不同身份) -> @Index (加快读写速度,提高性能,暂不支持Array<String> / ByteArray / Float / Double , IndexType.VALUE为使用字符串,而不是哈希值读写)
