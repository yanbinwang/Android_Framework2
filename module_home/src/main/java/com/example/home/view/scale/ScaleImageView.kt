...
class ScaleImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ImageView(context, attrs, defStyleAttr) {
    ...
    private fun fitImageToView() {
        ...
        var scaleY: Float = viewHeight.toFloat() / drawableHeight
        when (mScaleType) {
            ScaleType.CENTER -> {
                scaleY = 1f
                scaleX = scaleY
            ...
            if (view.state == State.NONE || view.state == State.DRAG || view.state == State.FLING) {
                when (event.action) {
                    ...
                    MotionEvent.ACTION_MOVE -> if (view.state == State.DRAG) {
                        ...
                        val fixTransX = getFixDragTrans(deltaX, viewWidth.toFloat(), view.getImageWidth())
                        val fixTransY = getFixDragTrans(deltaY, viewHeight.toFloat(), view.getImageHeight())
    ...
    private class ScaleListener(var view: ScaleImageView) : SimpleOnScaleGestureListener() {
        ...
        override fun onScale(detector: ScaleGestureDetector): Boolean {
            view.scaleImage(detector.scaleFactor.toDouble(), detector.focusX, detector.focusY, true)
    ...
    fun scaleImage(scale: Double, focusX: Float, focusY: Float, stretchImageToSuper: Boolean) {
        ...
        matrixs.postScale(deltaScale.toFloat(), deltaScale.toFloat(), focusX, focusY)
        fixScaleTrans()
    }
    ...
    class Fling(var view: ScaleImageView, velocityX: Int, velocityY: Int) : Runnable {
        ...
        init {
            ...
            val startX = view.m[Matrix.MTRANS_X] as? Int
            val startY = view.m[Matrix.MTRANS_Y] as? Int
            val minX: Int
            val maxX: Int
            val minY: Int
            val maxY: Int
        ...
        override fun run() {
            ...
            if (scroller?.computeScrollOffset().orFalse) {
                ...
                currX = newX
                currY = newY
                matrixs.postTranslate(transX.toFloat(), transY.toFloat())
                view.fixTrans()
                view.imageMatrix = matrixs
                compatPostOnAnimation(view, this)
            }
