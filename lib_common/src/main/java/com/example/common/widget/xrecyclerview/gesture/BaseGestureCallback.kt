...
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.ACTION_MODE_DRAG_MASK
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.ACTION_MODE_SWIPE_MASK
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.ACTION_STATE_DRAG
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.ACTION_STATE_IDLE
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.ACTION_STATE_SWIPE
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.ANIMATION_TYPE_DRAG
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.DIRECTION_FLAG_COUNT
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.END
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.LEFT
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.RIGHT
import com.example.common.widget.xrecyclerview.gesture.ItemDecorationHelper.Companion.START
...
abstract class BaseGestureCallback {
    ...
    companion object {
        ...
        fun hitTest(child: View?, x: Float, y: Float, left: Float, top: Float): Boolean {
            return x >= left // 触摸点x坐标 ≥ View左上角x坐标（不超出左边界）
                    && x <= left + child?.width.orZero // 触摸点x坐标 ≤ View右下角x坐标（不超出右边界）
                    && y >= top // 触摸点y坐标 ≥ View左上角y坐标（不超出上边界）
                    && y <= top + child?.height.orZero // 触摸点y坐标 ≤ View右下角y坐标（不超出下边界）
        }
    ...
    fun getAbsoluteMovementFlags(recyclerView: RecyclerView?, viewHolder: RecyclerView.ViewHolder?): Int {
        ...
        return convertToAbsoluteDirection(flags, recyclerView?.layoutDirection.orZero)
    }
    ...
    fun chooseDropTarget(selected: RecyclerView.ViewHolder?, dropTargets: MutableList<RecyclerView.ViewHolder>, curX: Int, curY: Int): RecyclerView.ViewHolder? {
        ...
        for (i in 0..<targetsSize) {
            ...
            if (dx > 0) {
                ...
                if (diff < 0 && target.itemView.right > selected?.itemView?.right.orZero) {
                    val score = abs(diff)
            ...
            if (dx < 0) {
                ...
                if (diff > 0 && target.itemView.left < selected?.itemView?.left.orZero) {
                    val score = abs(diff)
                    if (score > winnerScore) {
                        winnerScore = score
                        winner = target
            ...
            if (dy < 0) {
                ...
                if (diff > 0 && target.itemView.top < selected?.itemView?.top.orZero) {
                    val score = abs(diff)
                    if (score > winnerScore) {
                        winnerScore = score
                        winner = target
            ...
            if (dy > 0) {
                ...
                if (diff < 0 && target.itemView.bottom > selected?.itemView?.bottom.orZero) {
                    val score = abs(diff)
                    if (score > winnerScore) {
                        winnerScore = score
                        winner = target
    ...
    fun onMoved(recyclerView: RecyclerView?, viewHolder: RecyclerView.ViewHolder?, fromPos: Int, target: RecyclerView.ViewHolder?, toPos: Int, x: Int, y: Int) {
        ...
        if (layoutManager?.canScrollVertically().orFalse) {
            val minTop = target?.itemView?.let { child ->
                layoutManager?.getDecoratedTop(child)
            }.orZero
            if (minTop <= recyclerView?.paddingTop.orZero) {
                recyclerView?.scrollToPosition(toPos)
            }
            val maxBottom = target?.itemView?.let { child ->
                layoutManager?.getDecoratedBottom(child)
            }.orZero
            if (maxBottom >= recyclerView?.height.orZero - recyclerView?.paddingBottom.orZero) {
                recyclerView?.scrollToPosition(toPos)
            }
        }
    }
     ...
     */
    fun onDraw(c: Canvas, parent: RecyclerView, selected: RecyclerView.ViewHolder?, recoverAnimationList: MutableList<ItemDecorationHelper.RecoverAnimation>, actionState: Int, dX: Float, dY: Float) {
     ...
     * 默认空实现，子类可重写做上层装饰绘制
     */
    private fun onChildDraw(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
        val view = viewHolder.itemView
    ...
    fun onDrawOver(c: Canvas, parent: RecyclerView, selected: RecyclerView.ViewHolder?, recoverAnimationList: MutableList<ItemDecorationHelper.RecoverAnimation>, actionState: Int, dX: Float, dY: Float) {
        val recoverAnimSize = recoverAnimationList.size
        for (i in 0..<recoverAnimSize) {
            val anim = recoverAnimationList[i]
            c.withSave {
    ...
    private fun onChildDrawOver(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
    }
    // </editor-fold>
    ...
    private fun getMaxDragScroll(recyclerView: RecyclerView?): Int {
        if (mCachedMaxScrollSpeed == -1) {
            mCachedMaxScrollSpeed = recyclerView?.resources?.getDimensionPixelSize(R.dimen.item_touch_helper_max_drag_scroll_per_frame).orZero
        }
