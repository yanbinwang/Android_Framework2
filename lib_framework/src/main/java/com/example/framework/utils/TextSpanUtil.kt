 ...
 * .build()
 */
class ColorSpan(@field:ColorInt val color: Int) : SpanType {
...
/**
 * 点击链接的 Span
 * "我已阅读《用户协议》和《隐私政策》".setSpanFirst("《用户协议》",ClickSpan(object :LinkSpan(){
 *      override fun onLinkClick(widget: View) {
 *          "点击用户协议".logWTF
 *      }
 *  })).setSpanFirst("《隐私政策》",ClickSpan(object :LinkSpan(){
 *      override fun onLinkClick(widget: View) {
 *          "点击隐私政策".logWTF
 *      }
 *  }))
 ...
 */
class LinkSpan(@field:ColorInt private val colorInt: Int, private val isUnderlineText: Boolean = false, private val timeMS: Long = 500L, private val listener: () -> Unit) : ClickableSpan() {
