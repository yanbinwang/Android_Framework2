...
class GestureManager(context: Context) {
     ...
     * 4) 如果不想实现 OnGestureListener 接口中的所有方法，也可以继承 GestureDetector.SimpleOnGestureListener 类，它是一个包含了 OnGestureListener 所有方法的空实现类，开发者可以根据需求选择性地重写其中的手势处理方法
     */
    private var gestureDetector = GestureDetector(context, object : GestureDetector.OnGestureListener {
        override fun onDown(e: MotionEvent): Boolean {
            /**
             * 手指按下时的处理逻辑
             * 1) 让 onDown 返回 true，本质是告诉系统 “我要关注这个手势的后续动作”，而非 “拦截事件不让子 View 用”
             * 2) 它不影响子 View 的触摸事件，只确保系统会继续把 onScroll、onFling 等后续手势回调。
             * 3) 如果 onDown 返回 false，整个手势识别链条会直接断裂，可能会导致手指滑动时，onScroll、onFling、onLongPress 完全不触发；
             * 4) 但返回 true 后，onScroll 就能正常响应 —— 因为系统会认为 “你不关心这个手势”，直接跳过了后续识别。
             */
            return callback?.onDown(e).orTrue
        }

        override fun onFling(e1: MotionEvent?, e2: MotionEvent, velocityX: Float, velocityY: Float): Boolean {
            // 手指快速滑动时的处理逻辑
            return callback?.onFling(e1, e2, velocityX, velocityY).orFalse
        }
        ...
        override fun onScroll(e1: MotionEvent?, e2: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
            // 手指滑动时的处理逻辑
            return callback?.onScroll(e1, e2, distanceX, distanceY).orFalse
        }
        ...
        override fun onSingleTapUp(e: MotionEvent): Boolean {
            // 手指单点抬起时的处理逻辑
            return callback?.onSingleTapUp(e).orFalse
        }
    })
