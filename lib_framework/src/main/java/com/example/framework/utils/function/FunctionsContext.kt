 ...
 * @param res 布局资源id
 * @param root 父容器
 * @param attachToRoot 加载出来的布局，要不要立刻添加到 root（父容器）中，默认等于 root != null
 *
 * 1) attachToRoot = true
 *    把布局添加到 root 里面
 *    返回值：root 本身
 ...
 */
fun Context.inflate(@LayoutRes res: Int, root: ViewGroup? = null, attachToRoot: Boolean = root != null): View {
