 ...
 */
fun CoordinateConverter.convertAmap(sourceLatLng: LatLng, type: CoordinateConverter.CoordType = CoordinateConverter.CoordType.GPS): LatLng {
 ...
 * 移动到点集合的中心点
 */
fun AMap?.moveToCenterPoint(latLngList: MutableList<LatLng>, zoom: Float = 18f, anim: Boolean = false) {
    this ?: return
    moveToLatLng(CoordinateUtil.calculateCenterPoint(latLngList), zoom, anim)
}
...
//    marker?.title = extra
    marker?.snippet = extra // 使用摘录比title更规范些
 ...
 * 绘制多边形
 */
fun AMap?.addPolygonFill(latLngList: MutableList<LatLng>) {
    this ?: return
    // 声明多边形参数对象
 ...
 * 判断点是否在多边形内
 */
fun AMap?.isPointInPolygon(latLngList: MutableList<LatLng>, point: LatLng): Boolean {
    this ?: return false
