 ...
 *  1) 外层刷新使用了三方库refreshLayoutKernel，默认配置的情况下，只要拉出刷新，不主动调取finishRefreshing就不会收回去（内部handler不会主动发送收回）故而无需担心多次拉拽导致页数错误的问题
 *  2) 极端情况，比如二级三级页面需要被盖住的列表页面刷新，会通过广播先调取onRefresh方法（重要），然后再在回调监听内调取获取列表数据接口，且请求本身使用了协程，该协程会先被cancel，不会存在页数冲突问题
 */
class Paging {
    var hasRefresh = false // 是否刷新（内部判断）
    var currentPage = 1 // 当前页数
    var currentCount = 0 // 当前页面列表数据总数（提取RecyclerView的Adapter中的集合数）
